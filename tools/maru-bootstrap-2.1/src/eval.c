// last edited: 2012-5-20 by strangemonad on icarus
// forked from - last edited: 2011-12-13 14:12:22 by piumarta on emilia
...
typedef union Object Obj; // Forward declared because we depend on the `oop` typedef.
typedef Obj *oop;
typedef oop (*imp_t)(oop args, oop env); // function pointer to method implementation.
...
struct Expr     { oop name, defunc, ctx, profile; };
...
static inline ObjectType getType(oop obj)
{
#if (TAG_INT)
...
oop _checkType(oop obj, int type, char *name, char *file, int line)
{
  if (obj && !((long)obj & 1) && !ptr2hdr(obj)->used) {
    fatal("%s:%i: attempt to access dead object %s - %lu\n", file, line, name, obj);
  }
...
static oop concatString(oop str1, oop str2)
{
  // XXX UNSAFE - overflow
  // I know right cardinal sin - I'm sure this will bite me.

  size_t len1 = stringLength(str1);
  size_t len2 = stringLength(str2);
  oop newStr = _newString(len1 + len2);  GC_PROTECT(newStr);
  memcpy(get(newStr, String, bits),
         get(str1, String, bits), sizeof(wchar_t) * len1);
  memcpy(get(newStr, String, bits) + sizeof(wchar_t) * len1,
         get(str2, String, bits), sizeof(wchar_t) * len2); GC_UNPROTECT(newStr);
...
static oop oopAt(oop obj, int index)
{
  if (obj && !isLong(obj) && !GC_atomic(obj)) {
    // Allocated memory (GC) size where each slot is an oop
    int numSlots = GC_size(obj) / sizeof(oop);
    if ((unsigned)index < (unsigned)numSlots) {
      // oop is a pointer to the base addre of an oop-structure
      // that structure is really just a C-array of oop
      // i.e. cast `obj` (the base) to an oop * - a C-array of oops
      //
      // NOTE: Allow negative indexing into the GC header
...
static oop newExpr(oop defunc, oop ctx)
{
  oop obj= newOops(Expr);		GC_PROTECT(obj);
  set(obj, Expr,defunc,    defunc);
...
static int isGlobal(oop var)
{
  ...
  return (env != nil) && (getLong(get(env, Env, level)) == 0);
}
...
// Forward declare of our high-level read routine.
// It's buffered and returns a valid oop
//
// We call it oop_read() instead of simply read() so it
// doesn't clash with read() from <unistd.h>

static oop oop_read(FILE *fp);
...
static oop readList(FILE *fp, int delim)
{
  ...
  do {
    ...
    if (obj == s_dot) {
      // Handle splats in arg lists.
      // Skip the dot symbol itself and treat the next element
      // as a list (there can only be 1 more element)
      obj = oop_read(fp);
      if (obj == DONE) fatal("missing item after .");
      /* obj = oop_read(fp);*/
      if (oop_read(fp) != DONE) fatal("extra item after .");
      /* splat = set(tail, Pair ,tail, obj);*/
      found_splat = true;
      /* goto eof;*/
    }
...
static void doprint(FILE *stream, oop obj, int storing)
{
  ...
  switch (getType(obj)) {
	...
	doprint(stream, get(obj, Expr,name), storing);
      ...
      fprintf(stream, "=");
      doprint(stream, cadr(get(obj, Expr,defunc)), storing);
      break;
    }
    case Form: {
      fprintf(stream, "Form(");
...
static oop expand(oop expr, oop env)
{
  ...
  if (is(Pair, expr)) {
    ...
    if (is(Symbol, head)) {
      ...
      if (is(Variable, val)) {
        val = get(val, Variable, value);
      }

      if (is(Form, val) && (nil != get(val, Form, function))) {
        oop fn = get(val, Form, function); // local scope only, not GC protected
        oop args = newPairFrom(env, getTail(expr), expr); GC_PROTECT(args);
        head = apply(fn, args, nil);                      GC_UNPROTECT(args);
    ...
    // dyn-dispatch to the correct expander based on the `expr` type.
...
// expand list i.e. each element (think evlis from LISP 1.5)
static oop exlist(oop list, oop env)
{
  if (!is(Pair, list)) {
    // handle atoms as lists, just expand them safely.
    return expand(list, env);
  }
...
static oop apply(oop fun, oop arguments, oop ctx)
{
  ...
  switch (getType(fun)) {
    case Expr: {
      ...
      oop args=    arguments;
      oop defunc=    get(fun, Expr,defunc);				GC_PROTECT(defunc);
      oop env=     car(defunc);
      oop formals= cadr(defunc);
	...
	formals= getTail(formals); // xxx formals should be in env with fixed argument arity in defunc
...
static subr(set)
{
  ...
  if (is(Expr, val) && (get(val, Expr, name) == nil)) {
    // Expres are forms that know about the variable that points to them
    set(val, Expr, name, get(var, Variable, name));
  }

  if (isGlobal(var)) {
    // Global vars simply need setting at the root.
    return set(var, Variable, value, val);
  }

  // Non globals need to be set on their proper nested env.
  oop currEnv = get(ctx, Context, env);
  oop varsEnv = get(var, Variable, env);
  long delta = getLong(get(currEnv, Env, level)) - getLong(get(varsEnv, Env, level));
  oop cx = ctx; while (delta--) cx = get(cx, Context, home);
...
static int equal(oop lhs, oop rhs)
{
    ...
    case String:
      ans = (is(String, rhs) && !wcscmp(get(lhs, String,bits), get(rhs, String,bits)));
      break;
...
/*
 * Maps more or less to POSIX popen(3)
 * args - single element list with the full command string
 *
 * mode will always be 'rw'
 *
 * Result - a pipe oop that can read with (freadstr pipe)
 * Caller must (pclose pipe) rather than (close pipe)
 */
static subr(popen)
{
  oop cmd = getHead(args);
  if (!is(String, cmd)) { fprintf(stderr, "open: non-string argument: ");  fdumpln(stderr, cmd);  fatal(0); }

  FILE *pipe = popen(wcs2mbs(get(cmd, String, bits)), "r");
  if (!pipe) {
    return nil;
  }

  fwide(pipe, 1);
  return newLong((long)pipe);
}

/*
 * Closes a pipe opeend by popen (much like close
 * closes a regular pipe / file).
 *
 * Result - -1 on error 0 on success
 * Note: I'm not passing the erno because it's back enough
 * to be leaking C / POSIX broken control flow - but we need
 * something until we have better.
 *
 */
static subr(pclose)
{
  oop pipe = car(args);
  if (!isLong(pipe)) { fprintf(stderr, "pclose: non-pipe argument: ");  fdumpln(stderr, pipe);  fatal(0); }

  int ret = pclose((FILE *)getLong(pipe));
  return newLong(ret);
}


/*
 * Really primitive file -> string reading
 * reads characters wide
 * reads until eof
 * buffered in chunks.
 *
 * Use at your own peril, this may create a really large
 * in-memory string.
 * Better than nothing until we have better async IO
 * architecture.
 *
 */
static subr(freadstr)
{
  oop stream = car(args); GC_PROTECT(stream);
  if (!isLong(stream)) { fprintf(stderr, "freadstr: non-stream argument: ");  fdumpln(stderr, stream);  fatal(0); }

  FILE *file = (FILE *)getLong(stream); GC_UNPROTECT(stream);

  oop resultStr = newString(L""); //GC_PROTECT(resultStr);
  wchar_t text[256];
  while (!feof(file)) {
    wchar_t *result = fgetws(text, 256, file);
    printf("gots me some '%s'", wcs2mbs(result));
    if (result) {
      oop first = resultStr; //GC_PROTECT(first);
      oop newStr = newString(text); GC_PROTECT(newStr);
      oop cattedStr = newStr;  GC_UNPROTECT(newStr); //GC_UNPROTECT(first);
      
      //concatString(resultStr, newStr); GC_UNPROTECT(newStr); GC_UNPROTECT(first);*/
...
static subr(set_array_at)
{
  ...
  oop arr = getHead(args);
  oop idx = getHead(getTail(args)); if (!isLong(idx)) return nil;
  oop val = getHead(getTail(getTail(args)));
  return arrayAtPut(arr, getLong(idx), val);
}
...
static void replFile(FILE *stream, wchar_t *path)
{
  ...
  for (;;) {
    ...
    /* if (!root_env) {*/
    /*   root_env = newEnv(get(globals, Variable, value), 1, 0); GC_PROTECT(root_env);*/
    /* }*/
...
static void profilingDisable(int stats)
{
	...
	for (obj= GC_first_object();  obj;  obj= GC_next_object(obj)) {
		...
		case Expr: {
			...
			source=  cddr(get(obj, Expr,defunc));
		    }
		    break;
		}
		case Subr: {
...
#undef _do

  {
    struct { char *name;  imp_t imp; } *ptr, subrs[]= {
      ...
      { " popen",    subr_popen },
      { " pclose",   subr_pclose },
      { " freadstr", subr_freadstr },
      /* { " fork",     subr_fork },*/
      /* { " exec",     subr_exec },*/
      /*ø››››{ " syscall",  subr_syscall },*/
//    { " current-environment",	   subr_current_environment },
  ...
  fprintf(stderr, "maru-bootstrap-2.1\n\n");
  tmp= nil;
  while (--argc) {
    tmp= newPair(nil, tmp);
